...
public class PostgresReplicationConnection extends JdbcConnection implements ReplicationConnection {
    ...
    private PostgresReplicationConnection(
        ...
        super(
                ...
                null,
                () -> PostgresReplicationConnection.class.getClassLoader(),
                "\"",
                "\"");
    ...
    private ReplicationStream createReplicationStream(
            ...
            throws SQLException, InterruptedException {
        KBReplicationStream s;

        try {
            try {
                s =
                        startKBReplicationStream(
                                startLsn,
                                plugin.forceRds()
            ...
            if (e.getMessage().matches("(?s)ERROR: option .* is unknown.*")) {
                ...
                s = startKBReplicationStream(startLsn, messageDecoder::optionsWithoutMetadata);
        ...
        final KBReplicationStream stream = s;
        ...
        return new ReplicationStream() {
            ...
            public void read(ReplicationMessageProcessor processor)
                ...
                if (shouldSkipMessage(read, lastReceiveLsn, startLsn, walPosition)) {
                    return;
                }
            ...
            /**
             * Checks if message should be skipped. When Kingbase's replication stream
             * returns messages with LSNs that were not recorded during the WAL search
             * phase, we gracefully skip them instead of crashing.
             */
            private boolean shouldSkipMessage(
                    ByteBuffer read,
                    Lsn lastReceiveLsn,
                    Lsn startLsn,
                    WalPositionLocator walPosition) {
                try {
                    return messageDecoder.shouldMessageBeSkipped(
                            read, lastReceiveLsn, startLsn, walPosition);
                } catch (io.debezium.DebeziumException e) {
                    if (e.getMessage() != null
                            && e.getMessage().contains("not present among LSNs seen")) {
                        LOGGER.warn(
                                "LSN {} not found in WAL search set. "
                                        + "This can happen when Kingbase returns messages "
                                        + "from concurrent transactions. Skipping this message "
    ...
    private KBReplicationStream startKBReplicationStream(
            final Lsn lsn,
            BiFunction<
