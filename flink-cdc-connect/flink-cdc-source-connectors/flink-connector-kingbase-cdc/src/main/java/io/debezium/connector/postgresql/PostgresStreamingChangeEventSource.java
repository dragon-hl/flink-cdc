            ...
            if (walPosition.searchingEnabled()) {
                try {
                    ...
                    if (e.getMessage() != null
                        ...
                        LOGGER.warn(
                                "The offset LSN {} is no longer available in the WAL (it has been cleaned up). "
                                        + "Falling back to start streaming from the slot's current position. "
                                        + "This may result in data loss for changes that occurred between the saved LSN "
                                        + "and the current slot position.",
                                offsetContext != null ? offsetContext.lsn() : "unknown");
                        // Close the current stream and reconnect, starting from the slot's current
                        // position
                        stream.stopKeepAlive();
                        replicationConnection.reconnect();
                        // Start streaming without a specific LSN, letting the slot determine the
                        // starting point
                        final WalPositionLocator fallbackPosition = new WalPositionLocator();
                        replicationStream.set(
                                replicationConnection.startStreaming(fallbackPosition));
                        stream = this.replicationStream.get();
                        stream.startKeepAlive(
                                Threads.newSingleThreadExecutor(
                                        PostgresConnector.class,
                                        connectorConfig.getLogicalName(),
                                        KEEP_ALIVE_THREAD_NAME));
                        this.lastCompletelyProcessedLsn = stream.startLsn();
                        LOGGER.info(
                                "Successfully recovered and started streaming from slot position: {}",
