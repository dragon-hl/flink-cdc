...
package org.apache.flink.cdc.connectors.kingbase.source.reader;
...
/** The {@link RecordEmitter} implementation for Kingbase pipeline connector. */
public class KingbasePipelineRecordEmitter<T> extends IncrementalSourceRecordEmitter<T> {
    ...
    public KingbasePipelineRecordEmitter(
            ...
            if (isDataChangeRecord(element) || isSchemaChangeEvent(element)) {
                ...
                if (!alreadySendCreateTableTables.contains(tableId)) {
                    if (createTableEventCache.isEmpty()) {
                        // Fallback: dynamically create schema via JDBC when cache is empty.
                        // This can happen if Kingbase's catalog query returns no tables
                        // from Debezium's PostgresConnection.getAllTableIds().
                        try (PostgresConnection jdbc =
                                postgresDialect.openJdbcConnection()) {
                            sendCreateTableEvent(
                                    jdbc, tableId, (SourceOutput<Event>) output);
                        }
                    } else {
                        for (CreateTableEvent createTableEvent : createTableEventCache) {
    ...
    private void sendCreateTableEvent(
        ...
        Schema schema = KingbaseSchemaUtils.getTableSchema(tableId, sourceConfig, jdbc);
        output.collect(
